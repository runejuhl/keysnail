...
var prompt = function () {
    ...
    function createCompletionList() {
        ...
        else
        {
            ...
            var cellForSearch = gFlags ? [for (i of gFlags) if ((gFlags[i] & IGNORE) === 0) i] : null;
        ...
        fetch: {
            tabs: function (context) {
                return function (left, whole) {
                    ...
                    let collection = [for (x of Iterator(Array.slice(tabs)))
                      [x[1].image, x[1].label, x[1].linkedBrowser.contentDocument.URL]];
            ...
            javascript: function (context) {
                return function (currentText, text) {
                    ...
                    {
                        cc.collection =
                            [for (k of util.getAllPropertyNames(root))
                              getRow(root, k)].sort(cmp);
                        return cc;
                    }
                    else
                    {
                        ...
                        if (obj)
                        {
                            if (all)
                            {
                                cc.collection = [for (k of util.getAllPropertyNames(obj))
                                  getRow(obj, k, prefix)].sort(cmp);
                            }
                            else
                            {
